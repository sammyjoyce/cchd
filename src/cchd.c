...
static size_t write_callback(
    ...
    char *new_data = realloc(response_buffer->data, response_buffer->size + real_size + 1);
    if (new_data == NULL) {
        return 0;
    }
...
static char *read_input_from_stdin(void) {
    // Read all input from stdin into a dynamically growing buffer, but enforce an upper bound on
    // the total size to prevent denial-of-service from unbounded input. We use exponential growth
    // for the buffer to amortize reallocations while keeping a tight limit.
    ...
    while (1) {
        ...
        size_t bytes_to_read =
            remaining_capacity < INPUT_BUFFER_READ_CHUNK_SIZE
            ? remaining_capacity
        ...
        if (bytes_read < bytes_to_read) {
            // We have reached EOF or an error. Distinguish between them because errors must be
            // handled explicitly, while EOF is expected.
            if (ferror(stdin)) {
                free(buffer);
                return NULL;
            }
...
static yyjson_mut_val *build_event_object(yyjson_mut_doc *output_doc, yyjson_val *input_root) {
    // Construct the event metadata object with a unique ID, type, timestamp, and session details.
    // This standardizes the event format for the server protocol, ensuring all required fields are
    // present and providing defaults for missing optional fields to avoid downstream failures.
    yyjson_mut_val *event_object = yyjson_mut_obj(output_doc);
    if (event_object == NULL) {
        return NULL;
    }

    char event_id_buffer[32];
    snprintf(event_id_buffer, sizeof(event_id_buffer), "evt_%ld", time(NULL));
    if (!yyjson_mut_obj_add_str(output_doc, event_object, "id", event_id_buffer)) {
        return NULL;
    }
    ...
    const char *event_name = yyjson_is_str(event_name_value)
        ...
        : "Unknown";
    if (!yyjson_mut_obj_add_str(output_doc, event_object, "type", event_name)) {
        return NULL;
    }
    if (!yyjson_mut_obj_add_str(output_doc, event_object, "name", event_name)) {
        return NULL;
    }

    uint64_t timestamp_ms = (uint64_t)time(NULL) * 1000;
    if (!yyjson_mut_obj_add_uint(output_doc, event_object, "timestamp", timestamp_ms)) {
        return NULL;
    }
    ...
    const char *session_id = yyjson_is_str(session_id_value)
        ...
        : "unknown";
    if (!yyjson_mut_obj_add_str(output_doc, event_object, "session_id", session_id)) {
        return NULL;
    }
...
static yyjson_mut_val *build_data_object(yyjson_mut_doc *output_doc, yyjson_val *input_root) {
    // Extract and copy tool-related data if present. We perform a deep copy of the tool_input
    // object to ensure the mutable document owns all values, preventing lifetime issues or
    // references to the original immutable input document.
    ...
    if (yyjson_is_obj(tool_input_value)) {
        yyjson_mut_val *tool_input_copy = yyjson_val_mut_copy(output_doc, tool_input_value);
        if (tool_input_copy == NULL) {
            return NULL;
        }
        if (!yyjson_mut_obj_add_val(output_doc, data_object, "tool_input", tool_input_copy)) {
            return NULL;
        }
    }
...
static yyjson_mut_doc *transform_input_to_protocol_format(yyjson_doc *input_doc) {
    // Transform the input JSON to the server protocol format by adding a version, event metadata,
    // and data sections. This ensures compatibility and provides a consistent structure for
    // server-side processing. We build the output as a mutable document to allow efficient
...
static int send_request_to_server(
    ...
    // Send the JSON payload to the server with retries and exponential backoff to handle transient
    // network issues. We limit retries to prevent indefinite hanging and reset the response buffer
    // on each attempt to avoid data leaks or corruption from failed requests.
    ...
    for (int attempt = 0; attempt < max_attempts; attempt++) {
        ...
        struct curl_slist *http_headers = curl_slist_append(NULL, "Content-Type: application/json");
...
static int process_server_response(
    const char *response_data,
    char **modified_output_ptr,
    const Configuration *configuration
) {
    // Parse the server response to determine the decision (allow, block, modify) and handle
    // accordingly. In fail-open mode, invalid responses default to allow for liveness over
    // correctness; in fail-closed, they block to prioritize safety.
    *modified_output_ptr = NULL;

    yyjson_doc *response_doc = yyjson_read(response_data, strlen(response_data), 0);
    ...
    const char *decision = yyjson_is_str(decision_value) ? yyjson_get_str(decision_value) : "allow";
...
static void parse_command_line_arguments(int argc, char *argv[], Configuration *configuration) {
    // Parse configuration from environment and command-line arguments, with defaults for safety.
    // Environment takes precedence initially, but flags override to allow flexible runtime changes.
...
int main(int argc, char *argv[]) {
    ...
    yyjson_doc *input_json_document = yyjson_read(input_json_string, strlen(input_json_string), 0);
    ...
    yyjson_mut_doc *protocol_json_document = transform_input_to_protocol_format(input_json_document);
    ...
    char *protocol_json_string = yyjson_mut_write(protocol_json_document, 0, NULL);
