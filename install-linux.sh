#!/bin/bash
# Claude Hooks Dispatcher (cchd) - Linux Installation Script
# Copyright (c) 2025 Sam Joyce. MIT License.

# This script installs the cchd binary on Linux by downloading the latest release from GitHub,
...
# We detect the Linux distribution to provide better error messages and install missing dependencies.
# This function reads /etc/os-release which is standard on most modern Linux distributions.
detect_distribution() {
...
# We limit supported architectures to x86_64, aarch64, and armv7l to match common Linux hardware.
...
# We detect the libc type (glibc vs musl) to download the correct binary.
# This is important for Alpine Linux and other musl-based distributions.
detect_libc() {
...
# We assert curl is installed first since it's required for downloading.
    ...
    if ! command -v curl > /dev/null 2>&1; then
        ...
        case "$distro" in
            ...
            alpine)
                ...
                ;;
            *)
                echo -e "${COLOR_RED}❌ Please install curl manually${COLOR_NONE}"
                exit 1
                ;;
        esac
    fi
...
extract_download_url() {
    ...
    local download_url
    download_url=$(echo "$release_information" | grep -o "https://[^\"]*cchd-${platform_arch}-${PLATFORM}-${libc_type}${DOWNLOAD_FILE_EXTENSION}" | head -n 1)

    if [ -z "$download_url" ]; then
        echo -e "${COLOR_RED}❌ No release found for ${platform_arch}-${PLATFORM}-${libc_type}${COLOR_NONE}" >&2
...
verify_signature_if_possible() {
    ...
    if ! command -v minisign > /dev/null 2>&1; then
        ...
        echo "To enable verification: install minisign from your package manager"
        return 0
    fi
...
echo "🚀 Installing Claude Hooks Dispatcher (cchd) for Linux"
echo "=================================================="
echo
